...
describe('Tables: Postgres LocalFS Driver', function () {
    let app
    const options = { database: { host: 'localhost', port: 5432, ssl: false } }
    /**
    ...
    describe('init', function () {
        it('should throw if no database options provided', async function () {
            await driver.init(app, {}).should.be.rejectedWith('Postgres LocalFS driver requires database options to be provided')
        })
        it('should connect admin client and log info', async function () {
            ...
            app.log.info.calledWith('Postgres LocalFS driver initialized').should.be.true()
        })
    ...
    describe('shutdown', function () {
        it('should end admin client and log info', async function () {
            ...
            app.log.info.calledWith('Shutting down Postgres LocalFS driver').should.be.true()
        })
        it('should log debug if end fails', async function () {
            await driver.init(app, options)
            driver._adminClient.end.rejects(new Error('fail'))
            await driver.shutdown()
            app.log.debug.calledWithMatch('Error shutting down Postgres LocalFS driver:').should.be.true()
        })
    })
    ...
    describe('createDatabase', function () {
        it('should create a new table with correct options', async function () {
            ...
            // Ensure the table was created with correct credentials
            ...
            app.db.models.Table.create.firstCall.args[0].should.have.property('credentials')
            app.db.models.Table.create.firstCall.args[0].credentials.should.have.property('host', 'localhost')
            app.db.models.Table.create.firstCall.args[0].credentials.should.have.property('port', 5432)
            app.db.models.Table.create.firstCall.args[0].credentials.should.have.property('database', team.hashid)
            app.db.models.Table.create.firstCall.args[0].credentials.should.have.property('user', team.hashid)
    ...
    describe('destroyDatabase', function () {
        ...
        it('should throw if db does not exist in Table', async function () {
            app.db.models.Table.byId.resolves(null)
            await driver.init(app, options)
            await driver.destroyDatabase({ id: 1, hashid: 't1hash' }, 1).should.be.rejectedWith('Database t1hash does not exist')
        })
        it('should throw if db does not exist in Postgres', async function () {
            const team = { id: 1, hashid: 't1hash' }
            const dbObj = { destroy: sinon.stub().resolves() }
            app.db.models.Table.byId.resolves(dbObj)
            await driver.init(app, options)
