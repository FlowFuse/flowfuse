...
function fullDiscovery (client) {
    client.get.withArgs('/sources').resolves({
        ...
        data: [
            ...
            { name: sourcePlatform.name, type: 'mqtt', connector: connector.name },
            { name: sourceMcpRes.name, type: 'mqtt', connector: connector.name }
        ]
    })
...
describe('EMQX bridge-setup', function () {
    ...
    describe('removeBridge', function () {
        ...
        it('deletes rules → sources → actions → connector in that order', async function () {
            ...
            // 2 rules + 4 sources + 1 action + 1 connector = 8 deletes
            client.delete.callCount.should.equal(8)
            ...
            paths[4].should.match(/^\/sources\//)
            paths[5].should.match(/^\/sources\//)
            paths[6].should.match(/^\/actions\//)
            paths[7].should.equal(`/connectors/mqtt:${connector.name}`)
        })
