...
describe('Search API', function () {
    ...
    async function setupApp (config) {
        ...
        ericTeamMembership.permissions = {
            applications: {
                [TestObjects.AppTwo.hashid]: Roles.None
            }
        }
    ...
    describe('Search with Application RBAC', function () {
        before(async function () {
            await setupApp({
        ...
        it('search results filter for application rbac setting requires a team to be specified', async function () {
            const response = await search({ team: TestObjects.BTeam.hashid, query: 'app' }, TestObjects.tokens.eric)
            response.statusCode.should.equal(200)
            const result = response.json()
            // Should not get anything from AppTwo
            result.count.should.equal(7)
            result.results.should.have.length(7)
            const appTwoResults = result.results.filter(obj => /app-two/i.test(obj.name) || /Application Two/.test(obj.name))
            appTwoResults.should.have.length(0)
        })
    })
})
