...
describe('Device API', async function () {
    ...
    describe('Edit device details', async function () {
        ...
        describe('assign to application', function () {
            ...
            it('`@flowfuse/nr-assistant` is excluded from starter snapshot when ai platform flag is disabled', async function () {
                app.config.features.register('ai', false, true)
                const agentVersion = '1.11.0'
                const device = await createDevice({ name: 'Ad1a-ai-plat-off', type: '', team: TestObjects.ATeam.hashid, as: TestObjects.tokens.alice, agentVersion })
                await app.inject({
                    method: 'PUT',
                    url: `/api/v1/devices/${device.id}`,
                    body: { application: TestObjects.Application1.hashid },
                    cookies: { sid: TestObjects.tokens.bob }
                })
                const response = await app.inject({
                    method: 'GET',
                ...
                const result = response.json()
                result.should.have.property('modules').and.be.an.Object()
                result.modules.should.not.have.property('@flowfuse/nr-assistant')
                app.config.features.register('ai', true, true)
            })
            it('`@flowfuse/nr-assistant` is excluded from starter snapshot when ai team flag is disabled', async function () {
                // Disable enableAllFeatures so individual feature flags are respected
                const props = TestObjects.defaultTeamType.properties
                const origEnableAll = props.enableAllFeatures
                ...
                const agentVersion = '1.11.0'
                const device = await createDevice({ name: 'Ad1a-ai-team-off', type: '', team: TestObjects.ATeam.hashid, as: TestObjects.tokens.alice, agentVersion })
                await app.inject({
                    method: 'PUT',
                    url: `/api/v1/devices/${device.id}`,
                    body: { application: TestObjects.Application1.hashid },
                ...
                // Restore
                props.enableAllFeatures = origEnableAll
                props.features.ai = true
                TestObjects.defaultTeamType.properties = props
                await TestObjects.defaultTeamType.save()
            })
            it('`@flowfuse/nr-assistant` is removed from existing snapshot modules when ai is disabled', async function () {
                app.config.features.register('ai', false, true)
                const agentVersion = '1.11.0'
                const device = await createDevice({ name: 'Ad1a-ai-snap-off', type: '', team: TestObjects.ATeam.hashid, as: TestObjects.tokens.alice, agentVersion })
                await app.inject({
                    method: 'PUT',
                    url: `/api/v1/devices/${device.id}`,
                    body: { application: TestObjects.Application1.hashid },
                ...
                const dbDevice = await app.db.models.Device.byId(device.id)
                // Create a snapshot that includes nr-assistant in its modules
                const snapshot = await app.db.models.ProjectSnapshot.create({
                    name: 'snap-with-assistant',
                    description: '',
                    settings: {
                        settings: {},
                        env: {},
                ...
                await app.inject({
                    method: 'PUT',
                    url: `/api/v1/devices/${device.id}`,
                    body: { targetSnapshot: snapshot.hashid },
                    cookies: { sid: TestObjects.tokens.bob }
                })
