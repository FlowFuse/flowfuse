describe('FlowFuse - Brokers', () => {
    ...
    it('should allow users to configure third party brokers when no brokers exist and then delete it', () => {
        ...
        cy.get('[data-el="listbox-options"]').contains('MQTT').click()
        cy.get('[data-select="protocolVersion"]').click()
        cy.get('[data-el="listbox-options"]').should('be.visible')
        cy.get('[data-el="listbox-options"]').contains('3.0').click()
        cy.get('[data-select="ssl"]').click()
        cy.get('[data-el="listbox-options"]').should('be.visible')
        cy.get('[data-el="listbox-options"]').contains('Yes').click()
        ...
        cy.get('[data-input="host"]').type('host')
        cy.get('[data-input="port"]').type('1883')
        cy.get('[data-input="topic"]').type('topic')

        cy.get('[data-action="submit"]').should('not.be.disabled')
        cy.get('[data-action="submit"]').click()

        cy.wait('@createBroker')

        cy.get('[data-el="brokers-list"]').find('tr').should('have.length', 1)
        cy.get('[data-el="brokers-list"]').find('tr').contains('broker1')
        cy.get('[data-el="brokers-list"]').find('tr').contains('MQTT')
        cy.get('[data-el="brokers-list"]').find('tr').contains('3.0')
        cy.get('[data-el="brokers-list"]').find('tr').contains('Yes')
        cy.get('[data-el="brokers-list"]').find('tr').contains('Yes')
