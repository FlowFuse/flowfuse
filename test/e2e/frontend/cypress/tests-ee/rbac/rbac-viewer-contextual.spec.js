describe('FlowFuse - RBAC Viewer Contextual permissions', () => {
    let team
    let instances
    let devices
    let applications
    ...
    // groups
    it('should not have access to the device gorups page due to his team role', () => {
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-nav="device-groups"]').should('not.exist')
        cy.visit(`/team/${team.slug}/groups`)
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-page="team-device-groups"]').should('not.exist')
    })

    // pipelines
    it('should not have access to the team pipeline page', () => {
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-nav="team-pipelines"]').should('not.exist')
        cy.visit(`/team/${team.slug}/pipelines`)
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-page="team-pipelines"]').should('not.exist')
    })
    ...
    // brokers
    it('should not have access to brokers', () => {
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-nav="team-brokers"]').should('not.exist')
        cy.visit(`/team/${team.slug}/brokers`)
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-team="rbac-team"]#team-dashboard').should('exist') // redirected to home page
    })

    // performance
    it('should not have access to the team pipelines page', () => {
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-nav="team-performance"]').should('not.exist')
        cy.visit(`/team/${team.slug}/brokers`)
        cy.get('[data-el="loading"]').should('not.exist')
        cy.get('[data-team="rbac-team"]#team-dashboard').should('exist') // redirected to home page
    })
