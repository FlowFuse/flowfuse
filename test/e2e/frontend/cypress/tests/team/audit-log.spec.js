describe('FlowForge - Team Audit Log', () => {
    ...
    describe('FlowForge Team Audit Logs filtering', () => {
        ...
        it('enables filtering by a user', () => {
            ...
            cy.get('[data-el="listbox-options"] > .ff-option').eq(1).contains('Alice Skywalker').should('be.visible')
            cy.get('[data-el="listbox-options"] > .ff-option').eq(1).click()
            cy.wait('@getAuditLog')
            ...
            cy.get('[data-el="listbox-options"] > .ff-option').eq(2).contains('Bob Solo').should('be.visible')
            cy.get('[data-el="listbox-options"] > .ff-option').eq(2).click()
            cy.wait('@getAuditLog')
