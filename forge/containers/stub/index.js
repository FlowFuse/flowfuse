...
module.exports = {
    ...
    /**
     * Gets the features of the MCP endpoints generated by the mcp nodes in the running node-red instance.
     * @param {Project} project - the project model instance
    ...
    getMCPFeatures: async (project, endpoints) => {
        this._app.log.info(`[stub driver] Getting MCP features for ${project.id}`)
        return {}
    },

    /**
     * Calls a tool on the MCP endpoints generated by the mcp nodes in the running node-red instance.
     * @param {Project} project - the project model instance
     * @param {string|McpEndpointSpec} endpoint - MCP endpoint to call the tool on.
     * @param {string} name - name of the tool to call.
     * @param {Object} input - arguments to pass to the tool
     * @returns {Object} result of the tool call
     */
    callMCPTool: async (project, endpoint, name, input) => {
        this._app.log.info(`[stub driver] Calling MCP tool ${name} on ${project.id}`)
        return {}
    },

    /**
     * Reads a resource from the MCP endpoints generated by the mcp nodes in the running node-red instance.
     * @param {Project} project - the project model instance
     * @param {string|McpEndpointSpec} endpoint - MCP endpoint to call the tool on.
     * @param {string} uri - URI of the resource to read.
     */
    readMCPResource: async (project, endpoint, uri) => {
        this._app.log.info(`[stub driver] Reading MCP resource ${uri} on ${project.id}`)
        return {}
    },
