...
module.exports = async function (app) {
    ...
    app.post('/:deviceId/resources', {
        ...
        const team = await app.db.models.Team.byId(request.device.TeamId)
        setTimeout(() => {
            app.comms.devices.sendCommand(team.hashid, request.device.hashid, 'startResources', '')
            app.log.info(`Enable device resource ${request.device.hashid} in team ${team.hashid}`)
        }, 1000)
        reply.send(await app.db.controllers.BrokerClient.createClientForFrontend(request.device))
    })
    ...
    app.post('/:deviceId/generate/snapshot-description', {
        preHandler: [
            ...
            async (request, reply) => {
                ...
                const hasFeature = teamType.getFeatureProperty('generatedSnapshotDescription', false) || request.device.Team.getFeatureOverride('generatedSnapshotDescription')
