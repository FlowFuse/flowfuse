...
module.exports = {
    ...
    insertPlatformSpecificEnvVars: function (app, project, envVars) {
        ...
        if (app.config.features.enabled('customHostnames')) {
            ...
            const hostnameSetting = project.ProjectSettings?.find(settings => setting.key === KEY_CUSTOM_HOSTNAME)
    ...
    create: async function (
        ...
        if (flows && Array.isArray(flows) && flows.length > 0) {
            try {
                ...
                app.log.error(`Error creating flows for project ${instance.id} - ${e.stack}`)
            }
        }
