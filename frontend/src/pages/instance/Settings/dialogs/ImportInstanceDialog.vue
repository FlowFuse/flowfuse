<template>
    <ff-dialog ref="dialog" :header="$t('ui.importFlows')" :confirm-label="$t('ui.importInstance')" kind="danger" :disablePrimary="disabled" @confirm="confirm()" @cancel="cancel()">
        <template #default>
            <form class="space-y-6" @submit.prevent>
                <div class="mt-2 space-y-2">
                    <p class="text-sm text-gray-500">
                        {{ $t('ui.replaceCurrentFlowsByUploadingANewFlowFileThisWi') }}
                    </p>
                </div>
