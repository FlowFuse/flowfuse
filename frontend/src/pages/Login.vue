<template>
    <ff-layout-box class="ff-login">
        <div v-if="!pending" data-form="login">
            <ff-loading v-if="loggingIn" v-i18n message="Loading..." color="white" data-i18n="auth.loggingIn" />
            <template v-else-if="!mfaRequired">
                ...
                <div class="ff-actions">
                    <ff-button data-action="login" :disabled="loggingIn || tooManyRequests" @click="login()">
                        <span v-i18n data-i18n="auth.login">Login</span>
                        <span v-if="loggingIn || tooManyRequests" class="w-4">
                            <SpinnerIcon class="ff-icon ml-3 !w-3.5" />
                        </span>
                    </ff-button>
                    <ff-button v-if="settings['user:signup']" v-i18n kind="tertiary" to="/account/create" data-action="sign-up" data-i18n="auth.signup">Sign Up</ff-button>
                    <ff-button v-if="passwordRequired && settings['user:reset-password']" v-i18n kind="tertiary" :to="{'name': 'ForgotPassword'}" data-action="forgot-password" data-i18n="auth.forgotPassword">Forgot Password</ff-button>
                    <template v-if="googleSSOEnabled">
                        ...
                        <GoogleLogin class="w-full" :client-id="settings['platform:sso:google:clientId']" popup-type="TOKEN" :callback="ggCallback">
                            <ff-button class="w-full space-x-2" kind="secondary" data-action="google-login" :disabled="loggingIn">
                                ...
                                <span v-i18n data-i18n="auth.signInWithGoogle">Sign in with Google</span>
                                <span v-if="loggingIn || tooManyRequests" class="w-4">
                                    <SpinnerIcon class="ff-icon ml-3 !w-3.5" />
                                </span>
                            </ff-button>
            ...
            <template v-else>
                <label v-i18n data-i18n="auth.securityCodePrompt">Security Code</label>
                ...
                <div class="ff-actions">
                    <ff-button data-action="submit-token" :disabled="loggingIn || tokenInvalid" @click="submitMFAToken()">
                        <span v-i18n data-i18n="auth.continue">Continue</span>
                        <span class="w-4">
