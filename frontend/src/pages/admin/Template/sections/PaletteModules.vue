<template>
    <form class="space-y-4" @submit.prevent>
        <FormHeading>
            <div class="flex">
                <div v-if="editTemplate" class="mr-4">{{ $t('ui.defaultModules') }}</div>
                <div v-else class="mr-4">{{ $t('ui.installedModules') }}</div>
        ...
        <div v-if="!projectLauncherCompatible" class="text-red-400 space-y-1">
            <p>{{ $t('ui.youWillNeedToUpdateYourProjectStackToUseThisFeat') }}</p>
            <div v-if="project.stack.replacedBy">
                <ff-button size="small" to="./settings/danger">{{ $t('ui.update') }}</ff-button>
            </div>
        </div>

        <!-- text to display when editing a template -->
        <div v-if="editTemplate" class="text-gray-400 space-y-1">
            <p>{{ $t('ui.aListOfModulesThatWillBeIncludedInNewInstances') }}</p>
            <!-- additional text to display when palette_allowInstall is true -->
            <div class="space-y-1">
                <p>{{ $t('ui.changesToThisListWillNotAffectExistingInstancesT') }}</p>
                <p>{{ $t('ui.onlyCopiedToAnInstancesSettingsWhenItIsFirstCrea') }}</p>
            </div>
        </div>

        <!-- text to display when editing an instance -->
        <div v-else class="text-gray-400 space-y-1">
            <p>{{ $t('ui.thisIsTheListOfModulesThatWillBeInstalledIntoThe') }}</p>
            <!-- additional text to display when palette_allowInstall is true -->
            <div v-if="editable.settings.palette_allowInstall" class="space-y-1">
                <p>{{ $t('ui.anyChangesToThisListWillRequireRestartingTheInst') }}</p>
                <p>{{ $t('ui.youCanAlsoInstallModulesUsingThePaletteManagerIn') }}</p>
            </div>
        </div>
        ...
        <table class="w-full max-w-2xl table-fixed text-sm rounded-sm overflow-hidden">
            <thead>
                <tr class="font-medium">
                    <td class="border bg-gray-100 p-2 w-auto">{{ $t('ui.module') }}</td>
                    <td class="border bg-gray-100 p-2 w-48">{{ $t('ui.version') }}</td>
                ...
                <tr v-if="!readOnly && showAddRow" class="">
                    <td class="px-4 pt-4 border w-auto align-top">
                        <FormRow v-model="input.name" class="font-mono" :error="input.error" placeholder="@example/module">
                            <template #description>{{ $t('ui.npmModuleName') }}</template>
                        </FormRow>
                    </td>
                    <td class="px-4 pt-4 pb-3 border w-auto align-top space-y-3">
                        ...
                        <!-- <FormRow id="encrypt-env-var" v-model="input.encrypt" type="checkbox"> <span class="text-gray-500"><LockClosedIcon class="inline w-4" /> {{ $t('ui.encrypt') }}</span></FormRow> -->
                    </td>
